...
**✅ Testado e funcionando de verdade (2026-07-26).** `GOOGLE_API_KEY` configurada pelo usuário. Primeiro teste falhou (502) porque `gemini-2.5-flash` havia sido descontinuado para novas contas antes até da data de desligamento anunciada — corrigido para `gemini-3.5-flash`, o substituto oficial (ainda gratuito). Segundo teste: foto real de uma tarefa escolar → extraiu título, disciplina, data e prioridade corretamente, card verde de "pronta para importar". Detalhes em `docs/V5_ESPECIFICACAO_IMPORTACAO_POR_IMAGEM.md` seção 11.

**Lição a reter para qualquer integração de IA neste projeto:** provedores de IA descontinuam modelos com pouco ou nenhum aviso efetivo — se uma função de IA que funcionava começar a falhar 100% das vezes sem nenhuma mudança no nosso código, a primeira suspeita deve ser "o modelo ainda existe?", não um bug no nosso lado. Diagnóstico mais rápido: os logs padrão do Supabase (`get_logs`, service `edge-function`) só mostram status HTTP, não o corpo da resposta — para ver a mensagem de erro real é preciso inspecionar a aba Network do navegador (Response da chamada) ou adicionar `console.error()` explícito no código (já feito nesta função).
...
| 2026-07-23 | [Etapa 17 / Sessão 029n] Usuário curou a lista da v5: descartou assistente de estudos por IA, heatmap de conclusão e metas por período; e pediu um recurso novo que virou a **v4.0** (vem antes do registro por imagem): relatório mensal de acompanhamento enviado ao e-mail de um responsável cadastrado, todo dia 25, com edição/exclusão do e-mail exigindo autorização confirmada pelo próprio responsável. Criada `docs/V4_ESPECIFICACAO_RELATORIO_RESPONSAVEL.md` (3 tabelas + RLS, fluxo de token de uso único, 2 Edge Functions, pg_cron `0 8 25 * *`, conteúdo do relatório, checklist). Furo levantado e resolvido na mesma sessão: o pedido original não previa verificação no **cadastro**, só em editar/excluir — o que deixaria um e-mail errado sem correção possível. Usuário decidiu aplicar verificação nas 3 operações (cadastrar/editar/excluir) via **código de 6 dígitos** enviado ao responsável e digitado no app (o repasse do código é o consentimento), com validade 30min, uso único, máx. 5 tentativas, rate limit 60s e armazenamento só do hash. Provedor de e-mail decidido: **Resend** (integração simples em Deno + 3.000/mês grátis); pendência prática registrada — o app roda em subdomínio da Vercel e por isso não pode ser verificado como domínio remetente, então a estratégia é testar tudo com o domínio de teste do Resend antes de avaliar comprar domínio próprio. **v4.0 sem decisões pendentes, pronta para começar.** ROADMAP reorganizado com v4 no topo. Nenhum código — planejamento |
